...
export async function GET() {
  try {
    ...
    const rows = await withPrismaRetry(() => prisma.supplierProfile.findMany({
    ...
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "读取供应商失败" }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    if (!shortName || !fullName) return NextResponse.json({ ok: false, error: "简称和全称必填" }, { status: 400 });
    ...
    const data = {
      ...
      account_period_days: String(body.accountPeriodDays || "").trim() ? Number(body.accountPeriodDays) : null,
      enabled: Boolean(body.enabled ?? true),
    };

    if (id) {
      const target = await withPrismaRetry(() => prisma.supplierProfile.findFirst({
    ...
    const created = await withPrismaRetry(() => prisma.supplierProfile.create({
      data: { tenant_id: session.tenantId, company_id: session.companyId, ...data },
      select: { id: true },
    }));
    return NextResponse.json({ ok: true, id: created.id });
  } catch (error) {
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "保存供应商失败" }, { status: 500 });
  }
}
