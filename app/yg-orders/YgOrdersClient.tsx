...
type YgOrdersClientProps = { initialRows: ImportRow[]; };
...
export function YgOrdersClient({ initialRows }: YgOrdersClientProps) {
  ...
  async function handleChooseFile(file: File) {
    try {
      ...
      if (!response.ok || !result?.ok) throw new Error(result?.error || tx("导入失败", "Import fail"));
  ...
  async function saveEdit() {
    ...
    try {
      ...
      const result = await response.json();
      if (!response.ok || !result?.ok) throw new Error(result?.error || tx("保存失败", "Save fail"));
      ...
      setEditState(null);
    } catch (e) { setEditError(e instanceof Error ? e.message : tx("保存失败", "Save fail")); }
  ...
  return (
    <>
      <div className="grid gap-5">
          ...
          <div className="space-y-4 px-5 py-5">
            <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 px-5 py-6">
              <div className="flex flex-col items-center justify-center gap-4 text-center lg:flex-row lg:justify-center lg:gap-8">
                <div className="text-xs text-slate-500">{tx("接受导入格式 XLS 和 XLSX。", "Acepta XLS/XLSX")}</div>
        ...
        <TableCard title={tx("友购订单列表", "Pedidos YG")} description="" className="!mt-0">
          <div className="space-y-4 px-5 py-5">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div className="text-sm text-slate-500">{tx("按订单查看同步结果", "Ver resultado")}</div>
              <div className="w-full lg:w-[420px]"><input value={keyword} onChange={(e) => setKeyword(e.target.value)} placeholder={tx("搜索订单号、状态、客户、电话、备注", "Buscar")} className="h-10 w-full rounded-xl border border-slate-200 bg-white px-4 text-sm text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-primary/40" /></div>
            </div>
              ...
              <table className="w-full min-w-[1650px] border-separate border-spacing-0">
                <thead>
                  <tr className="bg-slate-50 text-left text-xs text-slate-500">
                    <th className="px-3 py-2.5 font-semibold text-slate-700">{tx("订单编号", "No")}</th><th className="px-3 py-2.5 font-semibold text-slate-700">{tx("订单状态", "Estado")}</th><th className="px-3 py-2.5 font-semibold text-slate-700">{tx("订单日期", "Fecha")}</th><th className="px-3 py-2.5 font-semibold text-slate-700">{tx("公司名称", "Empresa")}</th><th className="px-3 py-2.5 font-semibold text-slate-700">{tx("联系人", "Contacto")}</th><th className="px-3 py-2.5 font-semibold text-slate-700">{tx("联系电话", "Tel")}</th><th className="px-3 py-2.5 text-right font-semibold text-slate-700">{tx("订单金额", "Monto")}</th><th className="px-3 py-2.5 font-semibold text-slate-700">{tx("备注", "Nota")}</th><th className="px-3 py-2.5 text-right font-semibold text-slate-700">{tx("商品数量", "Items")}</th><th className="px-3 py-2.5 text-right font-semibold text-slate-700">{tx("供应商数量", "Prov")}</th><th className="px-3 py-2.5 font-semibold text-slate-700">{tx("第几门店", "Tienda")}</th><th className="px-3 py-2.5 font-semibold text-slate-700">{tx("操作", "Acciones")}</th>
                  </tr>
                </thead>
                <tbody className="text-[13px]">
                  {filteredRows.length === 0 ? <tr><td colSpan={12} className="px-3 py-10 text-center text-slate-500">{tx("暂无订单数据", "Sin datos")}</td></tr> : filteredRows.map((row) => {
                ...
                <section key={`${row.id}-split`} className="overflow-hidden rounded-xl border border-slate-200 bg-slate-50/60">
                  <div className="overflow-x-auto">
                    <table className="w-full min-w-[900px] border-separate border-spacing-0">
                      <thead><tr className="bg-slate-50 text-left text-xs text-slate-500"><th className="px-3 py-2.5 font-semibold text-slate-700">{tx("拆分订单号", "Split")}</th><th className="px-3 py-2.5 font-semibold text-slate-700">{tx("供应商", "Prov")}</th><th className="px-3 py-2.5 text-right font-semibold text-slate-700">{tx("订单金额", "Monto")}</th><th className="px-3 py-2.5 text-right font-semibold text-slate-700">{tx("商品数量", "Items")}</th><th className="px-3 py-2.5 text-center font-semibold text-slate-700">{tx("预览", "Ver")}</th><th className="px-3 py-2.5 text-center font-semibold text-slate-700">{tx("导出文件", "Export")}</th></tr></thead>
                      <tbody className="text-[13px]">
                        {row.supplierOrders.map((so) => (
                          <tr key={so.id} className="border-t border-slate-100 bg-white">
                            ...
                            <td className="px-3 py-2 text-center"><button type="button" onClick={() => setExportState({ importId: row.id, supplierOrderId: so.id })} className="inline-flex h-8 items-center justify-center rounded-lg border border-slate-200 bg-white px-3 text-xs font-semibold text-slate-700">{tx("导出文件", "Export")}</button></td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
      ...
      {detailState ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 px-4">
          <div className="w-full max-w-[1240px] rounded-xl bg-white shadow-2xl">
            <div className="border-b border-slate-200 px-5 py-4"><h3 className="text-base font-semibold text-slate-900">{tx("订单详情预览", "Detalle")}</h3><p className="mt-1 text-sm text-slate-500">{detailState.orderNo}</p></div>
            <div className="space-y-4 px-5 py-5">
              <div className="overflow-x-auto">
                <table className="w-full min-w-[1200px] border-separate border-spacing-0">
                  <thead><tr className="bg-slate-50 text-left text-xs text-slate-500"><th className="px-3 py-2.5 font-semibold text-slate-700">{tx("产品图片", "Img")}</th><th className="px-3 py-2.5 font-semibold text-slate-700">{tx("商品", "Producto")}</th><th className="px-3 py-2.5 font-semibold text-slate-700">{tx("条形码", "Cod")}</th><th className="px-3 py-2.5 font-semibold text-slate-700">{tx("供应商（位置）", "Ubic")}</th><th className="px-3 py-2.5 font-semibold text-slate-700">{tx("中文名", "CN")}</th><th className="px-3 py-2.5 font-semibold text-slate-700">{tx("西文名", "ES")}</th><th className="px-3 py-2.5 text-right font-semibold text-slate-700">{tx("数量", "Cant")}</th><th className="px-3 py-2.5 text-right font-semibold text-slate-700">{tx("单价", "Precio")}</th><th className="px-3 py-2.5 text-right font-semibold text-slate-700">{tx("普通折扣", "Desc")}</th><th className="px-3 py-2.5 text-right font-semibold text-slate-700">{tx("VIP折扣", "VIP")}</th><th className="px-3 py-2.5 text-right font-semibold text-slate-700">{tx("行金额 / 小计", "Subtotal")}</th></tr></thead>
                  <tbody className="text-[13px]">
              ...
              <div className="flex items-center justify-center gap-2"><button type="button" onClick={() => setPreviewPage((p) => Math.max(1, p - 1))} disabled={previewPage <= 1} className="inline-flex h-9 items-center justify-center rounded-xl border border-slate-200 bg-white px-4 text-sm font-medium text-slate-600 disabled:cursor-not-allowed disabled:opacity-40">{tx("上一页", "Ant")}</button><div className="inline-flex h-9 min-w-10 items-center justify-center rounded-xl border border-slate-300 bg-slate-50 px-3 text-sm font-semibold text-slate-700">{previewPage} / {previewTotalPages}</div><button type="button" onClick={() => setPreviewPage((p) => Math.min(previewTotalPages, p + 1))} disabled={previewPage >= previewTotalPages} className="inline-flex h-9 items-center justify-center rounded-xl border border-slate-200 bg-white px-4 text-sm font-medium text-slate-600 disabled:cursor-not-allowed disabled:opacity-40">{tx("下一页", "Sig")}</button></div>
            </div>
            <div className="flex justify-end border-t border-slate-200 px-5 py-4"><button type="button" onClick={() => setDetailState(null)} className="inline-flex h-10 items-center justify-center rounded-xl border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">{tx("关闭", "Cerrar")}</button></div>
          </div>
        </div>
      ) : null}
      {editState ? (
      ...
      {exportState ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 px-4">
          <div className="w-full max-w-[460px] rounded-xl bg-white shadow-2xl">
            <div className="border-b border-slate-200 px-5 py-4"><h3 className="text-base font-semibold text-slate-900">{tx("导出文件", "Export")}</h3></div>
            <div className="px-5 py-6"><div className="grid grid-cols-3 gap-4">
              ...
              <a href={`/api/yg-orders/${exportState.importId}/export/zip`} className="flex flex-col items-center justify-center rounded-2xl bg-white px-4 py-6 transition hover:bg-slate-50" onClick={() => setExportState(null)}><FileIcon label="ZIP" /><div className="mt-3 text-sm font-semibold text-slate-700">{tx("全部压缩包", "ZIP total")}</div></a>
            </div></div>
            <div className="flex justify-end px-5 py-4"><button type="button" onClick={() => setExportState(null)} className="inline-flex h-10 items-center justify-center rounded-xl border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">{tx("关闭", "Cerrar")}</button></div>
          </div>
        </div>
      ) : null}
    </>
