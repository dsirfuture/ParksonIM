...
export function ProductsManagementClient({
  ...
  async function exportComparePreview() {
    ...
    for (const r of comparePreview.rows) {
      ...
      // 上架/下架/新增列跟预览保持同色
      const onShelfCell = row.getCell(9);
          ...
          <table className="w-full min-w-[1400px] border-separate border-spacing-0">
            <thead className="sticky top-[64px] z-20 bg-slate-50">
              <tr className="bg-slate-50 text-left text-xs text-slate-500">
                <th className="whitespace-nowrap px-3 py-2.5 font-semibold text-slate-700">{tx("图片", "Img")}</th>
                <th className="whitespace-nowrap px-3 py-2.5 font-semibold text-slate-700">{tx("编码", "SKU")}</th>
                <th className="whitespace-nowrap px-3 py-2.5 font-semibold">{tx("条形码", "Barcode")}</th>
                <th className="whitespace-nowrap px-3 py-2.5 font-semibold text-slate-700">{tx("中文名", "ZH")}</th>
                <th className="whitespace-nowrap px-3 py-2.5 font-semibold text-slate-700">{tx("西文名", "ES")}</th>
                <th className="whitespace-nowrap px-3 py-2.5 text-right font-semibold">{tx("包装数", "Pk")}</th>
                <th className="whitespace-nowrap px-3 py-2.5 text-right font-semibold">{tx("装箱数", "Ct")}</th>
            ...
            <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
              <div>
                ...
                <div className="mt-1 flex items-center gap-4 text-xs text-slate-500">
                  ...
                  <p>导入SKU数量：{comparePreview.rows.length}（去重后：{compareSkuCount}）</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <button
